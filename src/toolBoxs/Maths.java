...
public class Maths {
	...
	public static float getDistance(Vector3f p, Vector3f e){
		float x=p.x-e.x;
		float y=p.y-e.y;
		float z=p.z-e.z;
		float distance=(float)Math.sqrt(x*x+y*y+z*z);
	...
	public static float getDistance(Player p, EntityPack e){
		float x=p.getPosition().x-e.getPosition().x;
		float y=p.getPosition().y-e.getPosition().y;
		float z=p.getPosition().z-e.getPosition().z;
		float distance=(float)Math.sqrt(x*x+y*y+z*z);
	...
	public static float getDistance(Player p,Vector3f position){
		float x=p.getPosition().x-position.x;
		float y=p.getPosition().y-position.y;
		float z=p.getPosition().z-position.z;
		float distance=(float)Math.sqrt(x*x+y*y+z*z);
		return distance;
	}
