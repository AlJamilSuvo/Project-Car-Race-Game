...
public class EntityRenderer {
	...
	public void render(Player player){
		List<TextureModel> textureModels=player.getModel();
	...
	public void render(EntityPack entityPack){
		List<TextureModel> textureModels=entityPack.getModel();
		for(TextureModel textureModel:textureModels){
			RawModel rawModel=textureModel.getRawModel();
			GL30.glBindVertexArray(rawModel.getVaoID());
			GL20.glEnableVertexAttribArray(0);
			...
			GL13.glActiveTexture(GL13.GL_TEXTURE0);
			GL11.glBindTexture(GL11.GL_TEXTURE_2D, textureModel.getModelTexture().getId());
			Matrix4f transformationMarix=Maths.createTransformationMatrix(entityPack.getPosition(), entityPack.getRotX(),
					entityPack.getRotY(), entityPack.getRotZ(), entityPack.getScale());
			shader.loadTransformationMatrix(transformationMarix);
			GL11.glDrawElements(GL11.GL_TRIANGLES, rawModel.getVertexCount(), GL11.GL_UNSIGNED_INT, 0);
			GL20.glDisableVertexAttribArray(0);
			GL20.glDisableVertexAttribArray(1);
